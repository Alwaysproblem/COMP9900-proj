...
def ShowBooking():
    ...
    show_line = 3
    L = []
    for i in range(len(posts)//show_line):
        L.append((i+1)*show_line)
    if(len(posts)%show_line != 0):
        L.append(len(posts))
    print(L)

    return render_template('showBooking.html', posts=posts, List = L, show = show_line)
...
def myRequests():
    ...
    sql = 'select * from requests where user_id = ' + str(current_user.ID)
    cur = query(sql)
    res = cur.fetchall()
    posts = [tupletodict(keys, tup) for tup in res]
    return posts
...
def my_requests():
    posts = myRequests()
    return render_template('my_request.html', request1=posts)
